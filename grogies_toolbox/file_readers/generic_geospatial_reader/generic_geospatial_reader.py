...
from grogies_toolbox.file_readers.generic_geospatial_reader.generic_csv import generic_csv_reader
from grogies_toolbox.file_readers.generic_geospatial_reader.generic_ersi import generic_esri_reader


def generic_geospatial_reader(file, encoding='ISO-8859-1'):
    if __is_shp(file):
...
def generic_geospatial_files(file_iterable, encoding='ISO-8859-1'):
    if isinstance(file_iterable, set) or isinstance(file_iterable, list):
        ...
        for file in file_iterable:
            files.append(generic_geospatial_reader(file, encoding))
        if len(files) == 1:
            return files[0]
        return files
    return generic_geospatial_reader(file_iterable)
